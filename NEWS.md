...
* `growth_auto_window()` derives the age range to fit over rather than requiring one: it opens at an age floor below which ground-plot programmes do not sample, and closes at the earliest of a quantile of observed plot ages, the end of the reference curve, and a fraction of `longevity` -- before LANDIS-II's senescence ramp drives the curve to zero. `growth_fitting_windows()` applies per-species overrides on top.
