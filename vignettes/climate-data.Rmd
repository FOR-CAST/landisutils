...
## Availability only -- deliberately NOT require()/library(). Attaching these merely to test for them
## is expensive and has side effects: BioSIM starts a J4R Java server on attach, which R CMD check
## then carries for the life of the vignette (and leaks if the check is interrupted). Since every
## chunk below is eval = FALSE, that cost bought nothing. system.file() answers the question without
## loading or attaching anything.
all_pkgs_avail <- all(nzchar(vapply(
  VignetteDepends,
...
which lets you pull wind (`ws`, `wnddir`) and shortwave-radiation (`srad`)
variables under the matching scenario - climr does not expose either, so
pair it with BioSIM when you need wind or radiation under a future
projection.
