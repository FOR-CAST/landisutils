...
- [`plot_growth_candidate()`](https://for-cast.github.io/landisutils/reference/plot_growth_candidate.md)
  ...
  effort; without that column it falls back to counts.
...
- [`simplifyCohorts()`](https://for-cast.github.io/landisutils/reference/simplifyCohorts.md)
  ...
  cases. Measured on one landscape, the initial communities carried a
  median 440 t/ha against 116 t/ha observed and exceeded the succession
  extension’s own `maxB` by four to thirteen times – a landscape the
  simulation has to shed before it can do anything else, and one that
  biases anything calibrated against its fuels. Shares are computed over
  the DISTINCT retained age classes, because
  [`prepInitialCommunities()`](https://for-cast.github.io/landisutils/reference/prepInitialCommunities.md)
  deduplicates its rows and anything spread across duplicates would be
  dropped with them.
