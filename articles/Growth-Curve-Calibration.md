...
## Four properties that keep the ranking honest
...
**Widen the plot pool by climate, one plot at a time.** Local plots are
usually too few to constrain every species, and the fix is to admit
plots from further afield in proportion to how much their climate
resembles the modelled landscape. Score each plot on its own
coordinates:

``` r

plots$distance <- growth_climatic_distance(plots, target = landscape_climate)
plots$w <- growth_climatic_weight(plots$distance, bandwidth = 0.5)

growth_reference_curves(reference, window, site = "site_identifier", weight = "w")
```

Resist the temptation to aggregate plots into map units (ecoregions,
climate zones, site types) and compare unit means instead. A unit’s
climate is only as well estimated as the number of plots inside it, and
the units with fewest plots get the noisiest estimates – so a ranking of
units is least reliable exactly where it is used, at the top. In the
network this was built against, resampling put a 14-plot unit’s rank
anywhere between 10th and 26th of 137, while a 95-plot unit sat within
four ranks. Per-plot distances have no such problem, and they yield a
continuous weight rather than an arbitrary “which units are in” cut.
