...
for the `ClimaticEx_Daily` model, fetching one calendar year for the
cells in `locations_batch[[1]]`. Output is appended to an Arrow CSV
dataset under
`path/ClimaticEx_Daily/<studyArea_hash>/<rcp>_<clim_model>/`,
...
- path:
  ...
  character. Directory under which the `ClimaticEx_Daily/` arrow dataset
...
- rcp:
  ...
  (BioSIM's own default).

- clim_model:

  character. BioSIM climate model. One of `"RCM4"`, `"GCM4"`,
  `"Hadley"`. Default `"RCM4"` (BioSIM's own default).
...
cache. For future years they select the CMIP5-era projection: `rcp` is
one of `"CONSTANT_CLIMATE"`, `"RCP45"`, or `"RCP85"`; `clim_model` is
one of `"RCM4"` (Canadian Regional Climate Model 4, CanESM2-driven),
`"GCM4"` (CGCM4 / CanESM2-family GCM), or `"Hadley"` (HadGEM).
