...
- quiet:
  ...
  The rewritten raster is written with the smallest pixel type LANDIS-II
  accepts that holds the largest new code: `INT1U` (GDAL `Byte`),
  `INT2S` (`Int16`) or `INT4S` (`Int32`). Its GDAL reader takes only
  byte, short, int, float and double, so the unsigned 16- and 32-bit
  types are rejected – as is `INT1S`, which GDAL 3.7+ writes as `Int8`.
  The type is pinned rather than inferred.
