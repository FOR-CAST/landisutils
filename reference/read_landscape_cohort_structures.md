...
which enumerates the cohort structures a landscape's initial communities
actually contain: one row per (structure, cohort).
...
A tibble with `structure_id`, `n_cohorts`, `species`, `cohort_age` and
`n_communities`.
