...
The hand-editable knobs that decide what the calibration is fitting to,
and over what part of the curve. This is the file to change when a fit
looks wrong; nothing here requires touching R.

## Usage

``` r
read_growth_scoring(path)
```
...
- path:
  ...
  Character. Path to `growth_scoring.csv`.
