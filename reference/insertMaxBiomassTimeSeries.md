...
config. This function writes `ForCS_MaxBiomassTimeSeries.csv` to `path`
...
``` r
insertMaxBiomassTimeSeries(df, path)
```
...
- df:
  ...
  data.frame corresponding to `MaxBiomassTimeSeries` table
