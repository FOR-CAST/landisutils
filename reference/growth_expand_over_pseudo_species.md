...
The ForCS tables are keyed on the species names in `species.txt`, which
for a factorial are pseudo-species. This maps each pseudo-species back
to its real species, joins the real species' values, and renames.
